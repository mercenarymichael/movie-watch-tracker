...
function Admin() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const accountResponse = await axios.get('http://localhost:8080/api/v1/accounts', {
                    headers: {
                ...
                const movieResponse = await axios.get('http://localhost:8080/api/v1/movies', {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    <div>
        ...
        >
            <Tab eventKey="accounts" title="Accounts">
                    ...
                    <tbody>
                        {accounts.map((account, index) => (
                            <tr key={index}>
                                ...
                                <td>{account.role}</td>
                                <td onClick={() => handleDelete(account)}>
                ...
                <button className="btn btn-primary" onClick={() => setKey("addAccount")}>
            ...
            <Table striped bordered hover>
                ...
                <button className="btn btn-primary" onClick={() => setKey("addMovie")}>
                    <i className="bi bi-plus-circle-fill"></i> Add
                </button>
            </Tab>
